...
numcolMydata = mydata.shape[1]
print("Number of Columns: "+str(numcolMydata))
numrowMydata = mydata.shape[0]
...
#tdf = mydata.groupby('domain')['ID'].nunique()

tdf = mydata.groupby((numcolMydata-1))[(numcolMydata-1)].nunique()

for i in tdf:
    print(i)

numClasses = tdf.shape[0]
print("Number of Classes = "+str(numClasses))
sortedData = mydata.groupby((numcolMydata-1))
#dict_of_compianies = {k: v for k, v in df.groupby('company_id')}
dict_of_classes = {keyClass: classval for keyClass, classval in mydata.groupby((numcolMydata-1))}
print(dict_of_classes.keys())
